...
const recuperarClave = async (data = {}) => {
  try {
    const response = await fetch(`${URL}/recover-password`, {
      method: 'POST', // *GET, POST, PUT, DELETE, etc.
      mode: 'cors', // no-cors, *cors, same-origin
      cache: 'no-cache', // *default, no-cache, reload, force-cache, only-if-cached
      credentials: 'same-origin', // include, *same-origin, omit
...
export const auth = { obtenerUsuarios, recuperarClave, verificarCorreo, iniciarSesion, crearCuenta, actualizarClave, getProfile, deleteUser, cerrarSesion }